...
begin
    ...
    if not new.raw_user_meta_data ? 'nickname' then
        raise exception 'Missing nickname in raw_user_meta_data';
    end if;

    if new.raw_app_meta_data ? 'provider' then
      if exists (select 1 from auth.users where (raw_user_meta_data ->> 'nickname')=(new.raw_user_meta_data ->> 'nickname')) then
        raise exception 'Nickname should be unique';
      end if;
    end if;

    if not new.raw_app_meta_data ? 'provider' then
        if new.raw_app_meta_data ->> 'nickname' != '익명' then
          raise exception 'Anonymous user nickname should be 익명';
        end if;
    end if;
...
GRANT ALL ON FUNCTION "auth"."validate_raw_user_meta_data"() TO "dashboard_user";
